...
object LookAtDnase extends Serializable with Logging {
  ...
  def run(sc: SparkContext, conf: EndiveConf): Unit = {
    ...
    val positiveCount = allData.filter(_.label > 0).count
    val negativeCount = allData.filter(_.label == 0).count

    val negsWithDnase = allData.filter(r => r.win.dnasePeakCount > 0 && r.label == 0).count()
    val posWithoutDnase = allData.filter(r => r.win.dnasePeakCount == 0 && r.label == 1).count()

    println(s"${totalCount}=${positiveCount}(pos)+${negativeCount}(neg)")
    println(s"negatives with some dnase = ${negsWithDnase}\n positives with no dnase: ${posWithoutDnase}")
  ...
  def featurizeWithWaveletDnase(sc: SparkContext,
    ...
    val approximators = Array(100, 50, 25, 12, 6)
      .map(r => {
        ...
        new KernelApproximator(W, FastMath.cos, dnaseSize, 1, fastfood=true)
      })
  ...
  def featurizeWithDnase(sc: SparkContext,
    ...
    val kernelApprox_dnase = new KernelApproximator(W_dnase, FastMath.cos, dnaseSize, 1, fastfood=true)
