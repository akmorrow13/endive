...
assert(cuts.count > 0)
  ...
  def featurize(sc: SparkContext,
    ...
    //println("number of positive:", rdd.filter(_.label > 0).count)
    //println("number of negative:", rdd.filter(_.label == 0).count)

    //val partitionedCuts: RDD[(ReferenceRegion, CutMap)] = coverage
