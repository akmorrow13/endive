...
files="$FWDIR"/target/scala-$SCALA_VERSION/endive-assembly-*.jar

if [ ${#files[@])} != 0 ]; then
  export ASSEMBLY_JAR=`ls "$FWDIR"/target/scala-$SCALA_VERSION/endive-assembly*.jar | tail -n 1`
  export DEPS_JAR=`ls "$FWDIR"/target/scala-$SCALA_VERSION/endive-assembly*.jar | head -n 1`
fi
